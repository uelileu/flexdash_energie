...
```{r}
biog <- read_sf("data/BiogasPlants.gpkg", layer="BiogasPlant") %>% st_transform(4326)
  
leaflet(biog) %>% 
  addTiles(attribution = "Quelle: Bundesamt für Energie") %>% 
  addCircleMarkers(color = "green", 
                   radius = ifelse(!is.na(biog$CombinedHeatAndPower),biog$CombinedHeatAndPower %>% as.numeric() %>% log(base = 2),1),
                   fill = T,
                   fillOpacity = 0.1,
                   opacity = 0.7,
                   popup = paste0("Betreiber: ", biog$Name,"<br>",
                                  "In Betrieb seit: ", biog$BeginningOfOperation, "<br>",
                                  "Leistung kombiniert: ", biog$CombinedHeatAndPower))
```
